/**
    ...
    \sa         \$HeadURL: https://svn.code.sf.net/p/sasunit/code/tags/v2.1/saspgm/sasunit/assertperformance.sas $
...
*/ /** \cond */
%MACRO assertPerformance(
    i_expected= /*expected value*/,
    i_desc = Check for runtime /*description of the assertion to be checked*/
);
    ...
    %LOCAL l_casid l_result l_errMsg l_macname;
    ...
    %IF %_handleError(
            ...
            (&l_casid = . OR &l_casid =),
            &g_error.(SASUNIT): assert must not be called before initTestcase) %THEN %DO;
        %RETURN;
    %END;
    ...
    PROC SQL NOPRINT;
        ...
        INTO: l_cas_runtime
        FROM target.cas
        WHERE cas_scnid = &g_scnid. AND cas_id = &l_casid.;
    QUIT;
