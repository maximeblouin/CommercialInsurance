/**
    ...
    \sa         \$HeadURL: https://svn.code.sf.net/p/sasunit/code/tags/v2.1/saspgm/sasunit/_createrepdata.sas $
...
%macro _createRepData(
    ...
    /*- Check for empty scenarios and generate entries in temporary copies of
        cas and tst datasets, in order to make scenario appear in report with
        a dummy test case */
    %LOCAL l_sEmptyScnDummyCasDesc;
    ...
    *** Check if all entries are present ***;
    ...
    %IF %_handleError(&l_macname.
                        ,ErrorTestDB
                        ,&syserr. NE 0
                        ,%nrstr(Error accessing the test database)
                        )
        %THEN %GOTO errexit;
    %GOTO exit;
